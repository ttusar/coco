 ...
 */
static coco_suite_t *suite_rw_splendor_initialize(void) {

  coco_suite_t *suite;
  const size_t dimensions[] = { 2, 3, 4 };

  suite = coco_suite_allocate("rw-splendor", 1, 3, dimensions, "instances: 1-15");
 ...
 */
static coco_problem_t *suite_rw_splendor_get_problem(coco_suite_t *suite,
  ...
  problem = rw_splendor_problem_allocate(suite->suite_name, 1, function, dimension, instance);
  assert(problem != NULL);
