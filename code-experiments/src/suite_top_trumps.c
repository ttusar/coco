/**
 * @file suite_top_trumps.c
 *
 * @brief Contains newly generated suite
 */
...
static coco_suite_t *suite_top_trumps_initialize(const char *suite_options) {
  ...
  suite = coco_suite_allocate("top-trumps", 5, 4, dimensions, "instances: 1-15");
...
static coco_problem_t *suite_top_trumps_get_problem(coco_suite_t *suite,
  ...
  const char *problem_id_template = "top-trumps_f%02lu_i%02lu_d%02lu";
  const char *problem_name_template = "single-objective toptrumps suite problem f%lu instance %lu in %luD";

  problem = top_trumps_problem_allocate(1, function, dimension, instance, problem_id_template, problem_name_template);
  assert(problem != NULL);
